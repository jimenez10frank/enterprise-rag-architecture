# ASSESSMENT.md — System Requirements

> This file is the source of truth. If anything in other docs contradicts this, this file wins. Do not paraphrase, summarize, or reinterpret it elsewhere.

## Enterprise RAG Architecture for the Tax Authority

### 1. The Scenario

The National Tax Authority is developing a secure, internal AI assistant for its employees (tax inspectors, legal counsel, and helpdesk staff). The system must flawlessly answer complex fiscal questions based on a massive corpus of 500,000 documents.
...
As the Lead AI Engineer, you are tasked with designing the architecture and specifying the exact configurations for this system. Please detail your design for the following four modules. Provide conceptual architecture, specific configuration parameters, and (pseudo-)code where applicable.

Your design should be concrete enough that a DevOps/AI engineering team can immediately begin implementation.

---

#### Module 1: Ingestion & Knowledge Structuring (Data Pipeline)
...
Queries will contain a mix of exact matches (e.g., "Ruling ECLI:NL:HR:2023:123") and semantic concepts (e.g., "deductibility of home office expenses").
...
- **Semantic Caching:** To reduce costs and latency for FAQs (e.g., "What is the Box 1 tax rate for 2024?"), design a Semantic Cache (e.g., using Redis). What cosine similarity threshold is safe for financial/tax data?
...
- AI usage is **encouraged**. Use Claude Code, Cursor, anything. The focus is on understanding what was built and being able to explain the choices.
- The output is a **GitHub repo** with working components AND an explanation of how AI was used.

## What this means for scope

We are not expected to actually run a 20M-chunk system. We are expected to:

1. Build a **working vertical slice** on a small corpus (~50 real legal documents) that demonstrates every module end-to-end.
2. Write a **design document** that specifies how the production system would be configured (HNSW params, quantization, reranker top-K, RBAC architecture, eval metrics with thresholds).
3. Write an **AI usage writeup** explaining what was generated, what was hand-written, and what was learned in the process.

Working code on small scale + design doc on production scale + honest AI usage doc = the three required artifacts.
